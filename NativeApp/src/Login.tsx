...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate("/test?redirectedFrom=login", { replace: true });
    } catch (err) {
      setError("An unexpected error occurred. Please try again.");
      setIsLoading(false);
    }
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 py-12 relative  bg-black/90">
      ...
      <img
        src="/ha.jpg"
        alt="Background"
        className={`absolute inset-0 h-full w-full object-cover ${darkMode ? "opacity-20" : "opacity-30"} pointer-events-none`}
      />
