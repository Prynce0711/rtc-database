...
import { exportSherriffExcel, uploadSherriffExcel } from "./ExcelActions";
import type { SherriffFilterOptions } from "./schema";
import {
  deleteSherriff,
  getSherriffsPage,
  getSherriffsStats,
...
type SherriffFilterValues = NonNullable<SherriffFilterOptions["filters"]>;

const toSherriffFilters = (filters: FilterValues): SherriffFilterValues => ({
  ...filters,
});
...
const SherriffRow = ({
  ...
  log: Sherriff;
  onEdit: (log: Sherriff) => void;
  onDelete: (log: Sherriff) => void;
  isAdminOrAtty: boolean;
  isSelected?: boolean;
  onToggleSelect?: (id: number) => void;
}) => {
  const popoverId = `sherriff-actions-popover-${log.id}`;
  const anchorName = `--sherriff-actions-anchor-${log.id}`;
  ...
  return (
    <tr className="bg-base-100 hover:bg-base-200 transition-colors cursor-pointer text-sm">
      {isAdminOrAtty && onToggleSelect && (
        <td className="text-center" onClick={(e) => e.stopPropagation()}>
          <input
            ...
            aria-label={`Select sheriff entry ${log.id}`}
          />
        </td>
      )}
      {isAdminOrAtty && (
      ...
      <td className="text-center">{log.name || "-"}</td>
      <td className="text-center">{log.mortgagee || "-"}</td>
      <td className="text-center">{log.mortgagor || "-"}</td>
      <td className="text-base-content/60 text-center">{log.remarks || "-"}</td>
    </tr>
  );
};

const SherriffLogsPage: React.FC = () => {
  const router = useRouter();
  const [logs, setLogs] = useState<Sherriff[]>([]);
  ...
  const [appliedFilters, setAppliedFilters] = useState<SherriffFilterValues>(
    {},
  );
  ...
  const sherriffFilterOptions: FilterOption[] = [
    { key: "ejfCaseNumber", label: "EJF Case Number", type: "text" },
    ...
    async (page = currentPage) => {
      try {
        ...
        if (!listResult.success) {
          setError(listResult.error || "Failed to fetch sheriff records");
          return;
        }
        ...
        setError(
          err instanceof Error
            ? err.message
            : "Failed to fetch sheriff records",
        );
      } finally {
        setLoading(false);
      }
  ...
  const handleSort = (key: SherriffSortKey) => {
    setSortConfig((prev) => ({
      key,
  ...
  const getSuggestions = async (
    ...
    const result = await getSherriffsPage({
      page: 1,
      pageSize: 10,
      filters: toSherriffFilters({ [key]: inputValue }),
      exactMatchMap: { [key]: false },
      sortKey: key as SherriffSortKey,
      sortOrder: "asc",
    });
    ...
    const values = result.result.items
      .map((item) => {
        const val = item[key as keyof Sherriff];
  ...
  const handleDeleteLog = async (logId: number) => {
    ...
    const result = await deleteSherriff(logId);
    if (!result.success) {
  ...
  const handleDeleteSelectedLogs = async () => {
    ...
    statusPopup.showLoading("Deleting selected sheriff records...");

    try {
      const results = await Promise.allSettled(
        selectedLogIds.map((id) => deleteSherriff(id)),
      );
  ...
  const handleImportExcel = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const result = await uploadSherriffExcel(file);
      ...
      <main className="w-full">
        <div className="mb-8">
          ...
          <p className="text-xl text-base-content/50 mt-2">
            Manage sheriff records and mortgage details
          </p>
          <div className="mt-3 inline-flex items-center gap-2 px-3 py-1.5 rounded-lg bg-info/10 border border-info/20 text-info text-xs font-medium select-none">
            ...
            <span>Use row selection for bulk edit/delete actions</span>
          </div>
        </div>
              ...
              <>
                ...
                <button
                  ...
                  onClick={async () => {
                    setExporting(true);
                    try {
                      const result = await exportSherriffExcel();
                      if (!result.success) {
                        statusPopup.showError(
                          result.error || "Failed to export sheriff records",
                        );
                        return;
                      }
                ...
                <button
                  ...
                  onClick={() => {
                    router.push("/user/cases/sheriff/add");
                  }}
                >
                  Add Entry
                </button>
                ...
                <div className="rounded-lg border border-primary/30 bg-primary/10 px-4 py-3 flex items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      onClick={() =>
                        router.push(
                          `/user/cases/sheriff/edit?ids=${selectedLogIds.join(",")}`,
                        )
                      }
                    >
                      Edit Selected
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
          {[
            ...
            {
              label: "Unique Names",
              value: (stats.uniqueNames ?? 0).toLocaleString(),
        ...
        <div className="bg-base-100 rounded-lg shadow overflow-x-auto">
          <Table
            ...
            onSort={(k) => handleSort(k as SherriffSortKey)}
            showPagination={false}
            renderRow={(log) => (
              <SherriffRow
                key={(log as unknown as Sherriff).id}
                log={log as unknown as Sherriff}
                isAdminOrAtty={isAdminOrAtty}
                onEdit={(item) =>
                  router.push(`/user/cases/sheriff/edit?id=${item.id}`)
                }
                onDelete={(item) => handleDeleteLog(item.id)}
                isSelected={selectedLogIds.includes(
                  (log as unknown as Sherriff).id,
                )}
                onToggleSelect={(id) =>
                  setSelectedLogIds((prev) =>
                    prev.includes(id)
